...
(define (errorS ast val_x)
  (let [(value-function ((eval `(λ(,'x) ,ast))val_x))]
    value-function))

(define (eval-tree ast val_x)
  (cond [(not(list? ast)) (if (number? ast)
                              ast
                              val_x)]
        [(and (list? ast) (= 3 (length ast))) ((eval (car ast)) (eval-tree (cadr ast) val_x) (eval-tree (caddr ast) val_x))]))

(define (deviation ast)
  ;;(foldl + 0 (map (λ(x y) (abs (- x y ))) (map (λ(x) (errorS ast x)) RANGE) SAMPLES)))
  (foldl + 0 (map (λ(x y) (abs (- x y ))) (map (λ(x) (eval-tree ast x)) RANGE) SAMPLES)))
...
(define (size-recursive ast)
  (cond 
    [(list? ast) (+ 1 (+ (size-recursive (cadr ast)) (size-recursive (caddr ast))))]
    [else (not(list? ast))  1])) 
...
;; Înlocuiește un subarbore aleator al unu(i AST cu un altul
